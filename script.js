...
if (!("WebSocket" in window)) {
    alert("你的浏览器不支持 WebSocket，无法建立连接！");
} else {
    // 让用户输入完整的服务器 URL（包括 IP 地址和端口）
    let serverUrl = prompt("请输入接收数据的服务器 URL（例如：http://192.168.1.100:5000 或 https://192.168.1.100:5000）");
    ...
    if (!serverUrl) {
        ...
        // 提取出协议、IP 地址和端口信息
        let urlPattern = /^(http|https):\/\/([\w\.-]+):(\d+)$/;
        let match = serverUrl.match(urlPattern);
        if (match) {
            let protocol = match[1];
            let serverIp = match[2];  // 提取服务器 IP
            let port = match[3];      // 提取端口号
            const wsProtocol = protocol === 'https' ? 'wss' : 'ws';
            const wsUrl = `${wsProtocol}://${serverIp}:${port}`; // 生成 WebSocket URL
            ...
            alert("服务器 URL 格式不正确，请按 http://ip:port 或 https://ip:port 的格式输入！");
        }
    }
